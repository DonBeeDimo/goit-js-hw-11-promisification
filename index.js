...
const myToggleUserState = (allUsers, userName) => {
  return new Promise((resolve, reject) => {
    const updatedUsers = allUsers.map(user =>
      user.name === userName ? { ...user, active: !user.active } : user,
    );
 ...
 * Сейчас работает так
 */
toggleUserState(users, 'Mango', myLogger);
toggleUserState(users, 'Lux', myLogger);

/*
 * Должно работать так
 */
myToggleUserState(users, 'Mango').then(myLogger);
myToggleUserState(users, 'Lux').then(myLogger);
...
const myMakeTransaction = transaction => {
  const delay = randomIntegerFromInterval(200, 500);
  return new Promise((resolve, reject) => {
 ...
 * Должно работать так
 */
myMakeTransaction({ id: 70, amount: 150 })
  .then(logSuccess)
  .catch(logError);

  myMakeTransaction({ id: 71, amount: 230 })
  .then(logSuccess)
  .catch(logError);

  myMakeTransaction({ id: 72, amount: 75 })
  .then(logSuccess)
  .catch(logError);

  myMakeTransaction({ id: 73, amount: 100 })
  .then(logSuccess)
  .catch(logError);